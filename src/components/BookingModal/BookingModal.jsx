...
export default function BookingModal({ service, onClose }) {
  ...
  const handleDateChange = (dayjsObj) => {
    const newDate = dayjsObj ? toLocalDateString(dayjsObj.toDate()) : '';
    if (!newDate || newDate < getTodayString() || newDate > getMaxDateString()) return;
    setDate(newDate);
  ...
  const disabledDate = (current) =>
    current < dayjs().startOf('day') || current > dayjs().add(6, 'month');
            ...
            <div className={s.timeRow}>
              <label className={s.label}>
                ...
                <Select
                  ...
                  onChange={(val) => { setTimeFrom(val); setTimeTo(''); }}
                  options={startSlots.map((slot) => ({ value: slot, label: slot }))}
                  placeholder="Начало"
                  disabled={allNight}
                  style={{ width: '100%' }}
                  getPopupContainer={() => document.body}
              ...
              <label className={s.label}>
                ...
                <Select
                  ...
                  options={endSlots.map((slot) => ({ value: slot, label: slot }))}
                  placeholder="Конец"
